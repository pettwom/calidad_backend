...
WHERE ar.rep_id = vcf.rep_id) descripcion,
(SELECT distinct ar.fk_id_estado
FROM cuestionarios.apk_replicas ar 
join cuestionarios.apk_estados ae on ae.id_estado = ar.fk_id_estado
WHERE ar.rep_id = vcf.rep_id) estado_id
from autenticacion.vw_calidad_filtro vcf  ) a  
    where a.estado_id != 7 and ${depto} ${mpio} ${com} ${ag} ${ae} ${emp};`
  );
  ...
  if (insertarRegistro) {
    ...
    switch (estado) {
      case 13:
        result = await con.query(
          ` INSERT INTO cuestionarios.apk_observaciones
                ...
                VALUES(1, ${ids}, 0, upper('${observacion}'), ${
            _user.id_usuario
      ...
      case 4:
        ...
        if (accion == 'edit') {
          console.log(` UPDATE "cuestionarios"."apk_observaciones" SET "obs_observacion" = '${observacion}',estado_id = 4, "obs_fecha_modificacion" = current_timestamp, "usumod_id" = ${
              _user.id_usuario
            }
             WHERE "obs_id" = ${d}`);
          
          result = await con.query(
            ` UPDATE "cuestionarios"."apk_observaciones" SET "obs_observacion" = '${observacion}',estado_id = 4, "obs_fecha_modificacion" = current_timestamp, "usumod_id" = ${
              _user.id_usuario
            }
             WHERE "obs_id" = ${d}`
          );
...
const getAlertas = async (req, res) => {
  ...
  if (verificarRep.rows[0].cant == 0) {
    console.log(`select * from calidad.fn_calidad_bucle(${rep_id}, 0 ,'${datosGenerales.rows[0].depto}', '${
        datosGenerales.rows[0].mpio
      }',${_user.id_usuario})`);
    
    await con.query(
      `select * from calidad.fn_calidad_bucle(${rep_id}, 0 ,'${datosGenerales.rows[0].depto}', '${
        datosGenerales.rows[0].mpio
      }',${_user.id_usuario})`
    );
    for (const a of getPreg.rows) {
      console.log(`select * from calidad.fn_validacion_calidad(${a.pre_numero_pregunta},${rep_id}, '${datosGenerales.rows[0].depto}', '${
          datosGenerales.rows[0].mpio
