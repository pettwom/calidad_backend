...
const getListado = async (req, res) => {
  ...
  var depto = req.params.depto != "null" ? ` a.cod_depto = '${req.params.depto}'` : "";
  var mpio = req.params.mpio != "null" ? ` and a.cod_municipio ='${req.params.mpio}'` : "";
  var com = req.params.com != "null" ? ` and a.cod_com ='${req.params.com}'` : "";
  var ag = req.params.ag != "null" ? ` and a.ag_unico = '${req.params.ag}'` : "";
  var ae = req.params.ae != "null" ? ` and a.ae_unico = '${req.params.ae}'` : "";
  var emp = req.params.emp != "null" ? ` and a.cod_empadronador = ${req.params.emp}` : "";
  ...
  console.log(`select row_number() over(order by a.cod_depto )nro, a.*, 
case when cas.estado_id is null then 0 else cas.estado_id end asignacion
from(
select distinct vcf.cod_depto , vcf.depto, vcf.cod_municipio, vcf.mpio, vcf.cod_com, vcf.comunidad, vcf.ag_unico,vcf.ae_unico, 
...
FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY rep_id ORDER BY id DESC) AS rn
FROM calidad.cal_asignacion) a 
join monitoreo.vw_usuarios vu on vu.aut_id_usuario = a.usu_asig_id
WHERE a.rn = 1 and a.rep_id = vcf.rep_id) nombre,
(SELECT distinct to_char(a.fecha_asignacion, 'dd-mm-yyyy') fecha_asignacion  
FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY rep_id ORDER BY id DESC) AS rn
FROM calidad.cal_asignacion) a 
join monitoreo.vw_usuarios vu on vu.aut_id_usuario = a.usu_asig_id
WHERE a.rn = 1 and a.rep_id = vcf.rep_id) fecha_asig,
(select ae.descripcion
from
(SELECT
    CASE
        WHEN ar.fk_id_estado BETWEEN 5 AND 13
        THEN (CASE 
                WHEN ar.fk_id_estado = ANY (ARRAY[5, 4, 7, 8, 10, 13])
                THEN ar.fk_id_estado
                ELSE maoc.id_estado --Esto asegura que se devuelva un valor si no coincide con el array
              END)
        ELSE maoc.id_estado
    END AS id_estado
FROM
    monitoreo.mon_aprobado_observacion_cuestionario maoc
    LEFT JOIN cuestionarios.apk_replicas ar ON ar.rep_id = maoc.rep_id
    JOIN cuestionarios.apk_estados ae ON ae.id_estado = maoc.id_estado
    where ar.rep_id = vcf.rep_id
     ) c
JOIN cuestionarios.apk_estados ae ON ae.id_estado = c.id_estado
where c.id_estado != 7 )  descripcion,
(select * from
(SELECT
    CASE
        WHEN ar.fk_id_estado BETWEEN 5 AND 13
...
FROM
    ...
    JOIN cuestionarios.apk_estados ae ON ae.id_estado = maoc.id_estado
    where ar.rep_id = vcf.rep_id
     )d where d.id_estado != 7) id_estado
from autenticacion.vw_calidad_filtro vcf 
) a 
left join calidad.cal_asignacion cas on cas.rep_id = a.rep_id
where  a.estado_rep != 7 and ${depto} ${mpio} ${com} ${ag} ${ae} ${emp};`
  );

  var result = await con_mon.query(
   `select row_number() over(order by a.cod_depto )nro, a.*, 
case when cas.estado_id is null then 0 else cas.estado_id end asignacion
from(
select distinct vcf.cod_depto , vcf.depto, vcf.cod_municipio, vcf.mpio, vcf.cod_com, vcf.comunidad, vcf.ag_unico,vcf.ae_unico, 
vcf.cod_empadronador, vcf.empadronador, vcf.cod_cuest, vcf.rep_id, vcf.estado_rep,
...
const VerificarEstados = async (depto, mpio, com, ag, ae, emp, id_usuario) => {
  var res_rep = await con_mon.query(`select a.rep_id
  ...
  var res_ubicacion = await con_mon.query(`select distinct a.depto::TEXT, a.mpio::TEXT
    from autenticacion.vw_calidad_filtro a 
    where  ${depto} ${mpio} ${com} ${ag} ${ae} ${emp}`);
...
const getValidar = async (req, res, next) => {
  await con.query(
    `UPDATE cuestionarios.apk_replicas set fk_id_estado = 3 , fecha_hora_aprobado = current_timestamp
...
const saveValidar = async (req, res) => {
  ...
  var preg_qwery = req.body.preg ? ` and ao.pre_num_pregunta = '${preg}'` : '';
  var pre_id = req.body.pre_id ? ` and pre_id = ${req.body.pre_id} ` : ''
  var accion = req.body.accion;
  var obs_id = req.body.obs ? req.body.obs : '';
  var obs_id_qwery = req.body.obs ? ` and obs_id = '${req.body.obs}'` : '';
  ...
  const estadoPreguntas = await con.query(` 
          SELECT DISTINCT ON (pre_id) pre_id, estado_id, obs_observacion, obs_id
          FROM cuestionarios.apk_observaciones ao 
          WHERE 
          tipo = 'PREGUNTA' 
          AND rep_id = ${ids} 
          ${preg_qwery}  ${obs_id_qwery} ${pre_id}
          OR (obs_id not in (select obs_id_mod
from cuestionarios.apk_observaciones ao 
where ao.estado_id = 7)and obs_id not in(select obs_id from cuestionarios.apk_observaciones ao where estado_id = 7)) 
          ORDER BY pre_id, ao.obs_id DESC`);
  console.log(` 
          SELECT DISTINCT ON (pre_id) pre_id, estado_id, obs_observacion, obs_id
          FROM cuestionarios.apk_observaciones ao 
          WHERE 
...
where ao.estado_id = 7)and obs_id not in(select obs_id from cuestionarios.apk_observaciones ao where estado_id = 7)) 
  ...
  console.log(tipoRep, '<=== tipoRep');
  console.log(estado, '<=== estado');
  console.log(estadoCuestionario.rowCount, '<=== estadoCuestionario.rowCount');
  console.log(pendiente.length, '<=== pendiente.length');
  ...
  if (insertarRegistro) {
      ...
      case 13:
        result = await con.query(
          ` INSERT INTO cuestionarios.apk_observaciones
                ...
                'server'::character varying, false, 4, '${tipo}',${estado}) RETURNING * `
        );
        break;
      case 7:
        console.log(tipoRep);
        if (tipoRep == 1) {
          ...
          // result = await con.query(
          //   `INSERT INTO cuestionarios.apk_observaciones
          //       (cue_id, rep_id, pre_id, usu_id, obs_justificacion, usucre_id, obs_eliminado, fecha_sincronizacion, id_sincronizacion, device_id, finalizado, estado_id, tipo, pre_num_pregunta)
          //       VALUES(1, ${ids}, ${preg}, 0, upper('${observacion}'), ${_user.id_usuario}, false, null, productores.generar_codigo_unico_sincronizacion(),
          //       'server'::character varying, false, ${estado}, '${tipo}',${preg}) RETURNING * `
          // );
          ...
          false, null, productores.generar_codigo_unico_sincronizacion(),
          'server'::character varying, false, ${estado}, '${tipo}','${preg}',${pendiente[0].obs_id}) RETURNING * `
          );
        }
        break;
      case 4:
        console.log(estadoPreguntas.rows, '<=== estadoPreguntas.rows');
        console.log(estadoCuestionario.rows, '<=== estadoCuestionario.rows');
    ...
    if (tipoRep == 1) {
      if (result.rowCount > 0) {
        console.log(` UPDATE cuestionarios.apk_replicas SET fk_id_estado= ${estado}, fecha_hora_aprobado = current_timestamp WHERE rep_id= ${ids}  `);

        const updateReplica = await con.query(
          ` UPDATE cuestionarios.apk_replicas SET fk_id_estado= ${estado} , fecha_hora_aprobado = current_timestamp WHERE rep_id= ${ids} `
        );
...
const saveAsignar = async (req, res) => {
  ...
  if (_user) {
    ...
    try {
      for (let a of cuest) {
        if (a) {
          var verificar = await con.query(`select count(1)cant
...
and usu_asig_id =${_user.id_usuario}`;
  ...
  // var observacion = await con.query(`select * from calidad.verificar_observacion_cuestionario()`)
  // var datos = []
  // var array1 = observacion.rows[0].verificar_observacion_cuestionario.split(', ');
  // var obser ={};
  // array1.map(a => {
  //   const [clave, valor] = a.split('-'); // Dividir por guion para obtener la clave y el valor
  //   obser[clave] = valor; // Asignar al objeto obser usando la clave como propiedad
  // });
  // console.log(obser)

  // datos.result = result.rows
  // datos.observacion = observacion.rows
  // console.log(datos);
...
const getAlertas = async (req, res) => {
  ...
  if (verificarRep.rows[0].cant == 0) {
    console.log(`select * from calidad.fn_calidad_bucle(${rep_id}, 0 ,'${datosGenerales.rows[0].depto}', '${datosGenerales.rows[0].mpio
      }',${_user.id_usuario})`);
    ...
    for (const a of getPreg.rows) {
      console.log(`select * from calidad.fn_validacion_calidad(${a.pre_numero_pregunta},${rep_id}, '${datosGenerales.rows[0].depto}', '${datosGenerales.rows[0].mpio
...
where ao.rep_id = ${rep_id} and ao.tipo ilike 'PREGUNTA' and ao.obs_id not in(select obs_id from cuestionarios.apk_observaciones ao where ao.obs_id in 
(select obs_id_mod from cuestionarios.apk_observaciones ao where ao.estado_id = 7)
or obs_id in ( 
select obs_id from cuestionarios.apk_observaciones ao where ao.estado_id = 7)) 
